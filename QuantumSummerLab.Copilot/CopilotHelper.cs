...
public class CopilotHelper : ICopilotHelper, IErrorSummarizer, IFeedbackTipper
{
    ...
    private static string BuildInstructions(ChatHistory chatHistory)
    {
        ...
        instructionsBuilder.AppendLine("You should never format your output, not even using markdown or asterisks, because the UI that shows your responses does not have support for this. Split-up every sentence with [BR] so it will be easier to display, but still use punctuation: So, [BR] after every dot, question mark or exclamation mark at the end of a sentence, no exceptions! Also always use ● for bullet points that are ALWAYS prepended by [BR] so that they start on a new line. This [BR]-after-every-sentence rule always applies to every part of your response outside of a code block, with no exceptions, even in messages that also contain a code block.");
        instructionsBuilder.AppendLine("Whenever you show Q# code, even a single line, keyword, operation, or variable name, wrap it in its own fenced code block using three backticks on a line by themselves before and after the code, exactly like this:\n```\ncode goes here\n```\nInside the fences, use real line breaks and indentation exactly as the code should be displayed, and never use [BR] or ● inside a code block. Never use single or double backticks, only ever this triple-backtick fenced form, every single time you show code, no exceptions.");
